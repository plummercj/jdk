...
void SuperWord::adjust_pre_loop_limit_to_align_main_loop_vectors() {
  ...
  _igvn.register_new_node_with_optimizer(old_limit);
  ...
  Node* constrained_limit =
    (stride > 0) ? (Node*) new MinLNode(_phase->C, new_limit, orig_limit)
                 : (Node*) new MaxLNode(_phase->C, new_limit, orig_limit);
