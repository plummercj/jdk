...
static bool invokeCallback(
    ...
    char* strBufBegin = NULL;
    char* strBufEnd = NULL;
    char* strBufNextChar = NULL;
    char* strNewBufBegin = NULL;
    size_t strBufCapacity = 0;
    int callbackMode = EXEC_CALLBACK_USE;
    int c;
    ptrdiff_t charOffset;
    int success = false;
    int tailCharCount = 0;
    ...
    for (;;) {
        ...
        if (EXEC_CALLBACK_USE != callbackMode) {
            ...
            if (tailCharCount) {
                JP_LOG_TRACE(
                        "execCommand: fetched %d trailing chars",
                        tailCharCount);
            }
            break;
        }
        ...
        if (strBufNextChar == strBufEnd) {
            /* Double buffer size */
            if (strBufCapacity) {
                strBufCapacity = strBufCapacity * 2;
            } else {
                strBufCapacity = 1;
            }

            if (strBufNextChar && strBufBegin) {
                charOffset = strBufNextChar - strBufBegin;
            } else {
                charOffset = 0;
            }

            strNewBufBegin = realloc(   strBufBegin,
            ...
            strBufNextChar = strNewBufBegin + charOffset;
